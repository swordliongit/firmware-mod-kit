...
function _wifi_lookup(ifn)
	...
	if radio and ifnidx then
		...
		uci_r:foreach("wireless", "wifi_iface",
			function(s)
	...
	elseif _wifi_iface(ifn) then
		...
		uci_s:foreach("wireless", "wifi_iface",
			function(s)
...
function get_wifidev(self, dev)
	if uci_r:get("wireless", dev) == "wifi_device" then
		return wifidev(dev)
	end
...
function get_wifidevs(self)
	...
	uci_r:foreach("wireless", "wifi_device",
...
function add_wifinet(self, net, options)
	if type(options) == "table" and options.device and
		uci_r:get("wireless", options.device) == "wifi_device"
	then
		local wnet = uci_r:section("wireless", "wifi_iface", nil, options)
		return wifinet(wnet)
	end
end
...
function wifidev.get(self, opt)
		-- tmp for enable device
	if opt == "enabled" then
		local enval = 0
		uci_r:foreach("wireless", "wifi_ctrl",function(s)
			 enval = s.enabled
			end)
		return enval
	end
...
function wifidev.get_wifinet(self, net)
	if uci_r:get("wireless", net) == "wifi_iface" then
		return wifinet(net)
	else
		local wnet = _wifi_lookup(net)
		if wnet then
...
function wifidev.del_wifinet(self, net)
	if utl.instanceof(net, wifinet) then
		net = net.sid
	elseif uci_r:get("wireless", net) ~= "wifi_iface" then
...
function wifinet.index(self)
	return uci_s:get("wireless", self.sid, "idx")
end
...
-- lan port message,current only link
-- open the sw_send_cmd first
local sw_patch = "/sbin/cs75xx/sw_scripts/"
function lannet_portinfo(self,port)
  local sw_infocmd = sw_patch .. "sw_get_mac_status.sh "
  local portinfo={}
  if port then 
    for l in utl.execi(sw_infocmd .. port) do
	if l:match("link") then
	   portinfo.link = tonumber(l:match("[^=]+=%s*0x(%d+)"))
	end	
    end
  end	   
  return portinfo  
