...
if not m.uci:get(arg[1]) == "rule2" then
	luci.http.redirect(m.redirect)
	return
end

s1 = m:section(NamedSection, arg[1], "defaults", translate("black list"))
s1.anonymous = true
s1.addremove = false
...
s = m:section(NamedSection, arg[1], "blackrule", translate("Url Filter"))
s.anonymous = true
s.addremove = false
...
function url.validate(self, value,section)
		...
		if old_value == value then
			...
			self.map.uci:foreach("firewall", "blackrule", function(section)
