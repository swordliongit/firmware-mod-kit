...
uci_apply_vendors() {
	customized=$(fw_printenv -n customized 2>/dev/null)
	[ "$customized" -ne 1 ] && {
		[ -f /vendor/etc/passwd ] && cp /vendor/etc/passwd /etc/passwd
		cd /vendor/uci-defaults || return 0
		fw_setenv customized 1
		files="$(ls)"
		[ -z "$files" ] && return 0
		mkdir -p /tmp/.uci
		for file in $files; do
