...
function macadd.validate(self, value,section)
		...
		uci:foreach("firewall", "mac_permit", function(section)
				 if section.src_mac == value    then
					valid = false
					m.message = translate("MAC has used by white list ,please re-enter other MAC")
				end
			end)
...
function m.handle(self, state, data)
  ...
  --cursor:load("firewall")
  	-- uci:section("firewall","mac",nil,{src_mac = data.src_mac, dir = data.dir})
       ...
       local newmac = uci:section("firewall","mac_deny")
	if data.src_mac then
  	...
  	if data.name then 
  	 ...
  	 uci:save("firewall")
        ...
        luci.http.redirect(luci.dispatcher.build_url("customer/security/macf"))
