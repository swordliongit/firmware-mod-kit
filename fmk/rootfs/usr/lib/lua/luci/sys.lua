...
function syslog()
	...
	filepath = filepath or "/var/log/messages"
...
--@ return Local Machine Interfaces' IPs
--[[function net.localIPs()
	local IPs = {}
	for k,v in ipairs(nixio.getifaddrs()) do
		if v.family == "inet" then -- get ipv4 
			IPs[#IPs+1] = v.addr
		end
	end
	return IPs
end]]
function net.conntrack_static()
	local connt = {}
	local IPs = {}
	local item
	for k,v in ipairs(nixio.getifaddrs()) do
	...
	if fs.access("/tmp/.nf_conntrack", "r") then
		for line in io.lines("/tmp/.nf_conntrack") do
			line = line:match "^(.-( [^ =]+=).-)%2"
			local entry, flags = _parse_mixed_record(line, " +")
			item = true
			for i,j in pairs(IPs) do
				if entry.src == j or entry.dst == j then
					item = false	
					break
				end
			end
			if item and flags[6] ~= "TIME_WAIT" then
				entry.layer3 = flags[1]
				entry.layer4 = flags[3]
				entry.timeout = flags[5]
				for i=1, #entry do
