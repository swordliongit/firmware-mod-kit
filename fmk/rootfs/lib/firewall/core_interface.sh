...
fw__uci_state_del() {
 ...
 fw_configure_interface() {
	...
	[ "$action" == "add" ] && {
		local status=$(uci_get_state network "$iface" up 0)  	### get up state from /var/state/network, status=1 if up
		[ "$status" == 1 ] || [ -n "$aliasnet" ] || return 0	### if down return.
	}

	[ -n "$ifname" ] || {								### ifname no null.
		ifname=$(uci_get_state network "$iface" ifname)	### lo, eth0, eth1, etc.. 
		ifname="${ifname%%:*}"
		[ -z "$ifname" ] && return 0
	}
	...
	fw_callback pre interface							### not any callback
	...
	config_get old_zones core "${iface}_zone"				### no core section, so old_zones is null
	[ -n "$old_zones" ] && {
	...
	[ -z "$aliasnet" ] && {						### if no alias
		...
		for a in $aliases; do
			...
			[ -n "$ipaddr" ] && fw_configure_interface "$a" add "" "$ipaddr${netmask:+/$netmask}"   ### nesting call
