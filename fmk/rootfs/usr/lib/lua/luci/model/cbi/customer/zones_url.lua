...
m.redirect = ds.build_url("customer", "security", "urlf")
...
--s.extedit   = ds.build_url("customer", "security", "urlf", "third1", "%s")
...
function s.create(self, section)
	...
	luci.http.redirect(ds.build_url(
		"customer", "security", "urlf", "rule1", created
	))
	return
end
...
--s.extedit   = ds.build_url("customer", "security", "urlf", "third2", "%s")
--s.defaults.target = "ACCEPT"
...
function s.create(self, section)
	...
	--m.uci:save("firewall")
	luci.http.redirect(ds.build_url(
		"customer", "security", "urlf", "rule2", created
	))
	return
end
