...
m1 = SimpleForm("firewall", translate("White Name"))
m1.redirect = dsp.build_url("customer", "security", "urlf")
...
function url.validate(self, value, section)
		...
		uci:foreach("firewall", "url_deny", function(section)
				if section.url == value    then
					valid = false
					m.message = translate("URL has used by black list ,please re-enter other URL ")
				end
			end)
...
function m.handle(self, state, data)
	if state == FORM_VALID then   
       ...
       local new_url = uci:section("firewall","url_permit")
	...
	if data.url then
  	...
  	if data.name then
  	 ...
  	 uci:save("firewall")
        ...
        luci.http.redirect(luci.dispatcher.build_url("customer/security/urlf"))
